...
/// Regenerates `index.md` in the words folder — a glanceable overview of all captured words.
///
...
enum WordIndex {
    ...
    static func regenerate(folderURL: URL, vaultPath: String) {
        ...
        // 2. Scan .md files in words folder, excluding index.md
        let mdFiles: [URL]
        do {
            ...
            ).filter { $0.pathExtension == "md" && $0.lastPathComponent.lowercased() != "index.md" }
        } catch {
            return
        }
        ...
        let tmpURL = folderURL.appendingPathComponent(".wh-index-\(UUID().uuidString).tmp")
        do {
